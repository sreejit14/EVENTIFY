...
function CheckoutModal({ cartItems, onClose, onPaymentSuccess }) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // ✅ Fixed: create one booking per cart item with correct fields
      const bookingPromises = cartItems.map((item) =>
        axios.post('/api/bookings', {
          vendorId:       item._id,
          vendorName:     item.name,
          vendorCategory: item.category,
          price:          String(getVendorEstimatedPrice(item.priceRange)),
        }, {
      ...
      alert(`Payment of ₹${totalAmount.toLocaleString("en-IN")} processed successfully via ${selectedPayment}! Booking confirmed.`);
      onPaymentSuccess();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4">
            ...
            <div className="space-y-2">
              {cartItems.map((item, index) => (
                <div key={`${item.name}-${index}`} className="flex justify-between text-sm">
                  <span>{item.name}</span>
                  <span>₹{getVendorEstimatedPrice(item.priceRange).toLocaleString("en-IN")}</span>
                </div>
              ))}
            </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <PaymentOption id="card"       label="Credit/Debit Card" description="Visa, Mastercard, RuPay"        colorClassName="blue"   selectedPayment={selectedPayment} onSelect={setSelectedPayment} />
            <PaymentOption id="upi"        label="UPI"               description="Google Pay, PhonePe, Paytm"    colorClassName="green"  selectedPayment={selectedPayment} onSelect={setSelectedPayment} />
            <PaymentOption id="netbanking" label="Net Banking"       description="All major banks"               colorClassName="purple" selectedPayment={selectedPayment} onSelect={setSelectedPayment} />
            <PaymentOption id="wallet"     label="Digital Wallet"    description="Paytm, Mobikwik, Ola Money"    colorClassName="orange" selectedPayment={selectedPayment} onSelect={setSelectedPayment} />
          </div>
        </div>
    ...
    <div className="mb-6">
      ...
      {selectedPayment === "netbanking" && (
        <select className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500">
          ...
          <option>Axis Bank</option><option>Other Banks</option>
        </select>
      )}
      {selectedPayment === "wallet" && (
        <select className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500">
          ...
          <option>Paytm</option><option>Mobikwik</option>
          <option>Ola Money</option><option>Amazon Pay</option>
        </select>
      )}
    </div>
  );
